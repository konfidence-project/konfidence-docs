...
A landscape is a logical group of stages, [deployment targets](../reference/glossary.md#deployment-target), and the resources that support their deployments. The resources in one landscape share an operational context.
...
- The infrastructure and [deployers](../reference/glossary.md#deployer) available to the application.
...
[Projects](../reference/glossary.md#project) provide the organizational context. Landscapes define operational boundaries within that context. Deployment targets connect each landscape to concrete infrastructure, while stages select the vectors to deliver there.
...
Stages in the same landscape can also [share deployments of reusable artifacts](../develop-integrate/artifact-types/index.md#choose-whether-vectors-share-one-instance-of-your-artifact). For example, if two team vectors contain the same stable database [artifact](../reference/glossary.md#artifact), Konfidence can reuse that deployment instead of creating one copy per stage. Separating the production landscape prevents this sharing from crossing the intended operational boundary.
...
Applications represented by different vectors often have many artifacts in common. Stages in one landscape can reuse compatible [artifact deployments](../reference/glossary.md#artifactdeployment), which avoids unnecessary copies while each stage still selects its own vector.
...
- [Deployment model](./deployment-model.md) explains how artifacts, [deployment classes](../reference/glossary.md#deployment-class), deployers, and targets connect.
